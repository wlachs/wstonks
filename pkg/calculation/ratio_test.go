...
// TestGetAssetRatio runs the smoke test.
func (suite *ratioTestSuite) TestGetAssetRatio() {
...
// TestGetAssetRatio_One_Sum runs the smoke test and makes sure that the sum of asset ratios add up to 1.
func (suite *ratioTestSuite) TestGetAssetRatio_One_Sum() {
	m, err := suite.ctx.GetAssetRatio(suite.ctx.AssetContext.Assets)
...
// TestGetAssetRatio_No_Transactions runs tests without any transactions.
func (suite *ratioTestSuite) TestGetAssetRatio_No_Transactions() {
...
// TestGetAssetRatio_No_Assets runs the test with no asset argument.
func (suite *ratioTestSuite) TestGetAssetRatio_No_Assets() {
	_, err := suite.ctx.GetAssetRatio([]*asset.Asset{})
	assert.EqualError(suite.T(), err, "sum of asset worth is zero", "should throw an error if assets have no worth")
}

// TestGetAssetRatio_Zero_Portfolio_Worth runs the test with a complete loss to make sure the correct error is returned
func (suite *ratioTestSuite) TestGetAssetRatio_Zero_Portfolio_Worth() {
...
// TestGetAssetRatio_No_Context runs the test without transaction and asset contexts.
func (suite *ratioTestSuite) TestGetAssetRatio_No_Context() {
